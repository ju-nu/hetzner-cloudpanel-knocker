...
# Extract existing rules, remove the rule with the specified port 8443 and 22
...
for port in 80 443; do
    rule=$(jq -n --arg port "$port" --arg ip4 "0.0.0.0/0" --arg ip6 "::/0" '{"direction": "in", "protocol": "tcp", "port": $port, "source_ips": [$ip4, $ip6], "destination_ips": []}')
    rules_ports_80_443+=("$rule")
done

# Create ICMP rules for all IPv4 and IPv6
rules_icmp=()
for protocol in icmp; do
    rule=$(jq -n --arg protocol "$protocol" --arg ip4 "0.0.0.0/0" --arg ip6 "::/0" '{"direction": "in", "protocol": $protocol, "source_ips": [$ip4, $ip6], "destination_ips": []}')
    rules_icmp+=("$rule")
done

# Combine all the new rules
all_new_rules=$(jq -n --argjson rule1 "$new_rule_8443" --argjson rule2 "$new_rule_22" --argjson rule3 "$(echo "${rules_ports_80_443[@]}" | jq -s '.')" --argjson rule4 "$(echo "${rules_icmp[@]}" | jq -s '.')" '$rule3 + [$rule1, $rule2] + $rule4')
