...
# Extract existing rules, remove the rule with the specified port
existing_rules=$(echo "$firewall_details" | jq '.firewall.rules')
filtered_rules=$(echo "$existing_rules" | jq 'map(select(.port != "'"$PORT"'" and .protocol != "tcp"))')
...
new_rule_8443=$(jq -n --arg port "$PORT" --arg ip "$NEW_IP" '{"direction": "in", "protocol": "tcp", "port": $port, "source_ips": [$ip], "destination_ips": []}')

# Create rules for all IPv4 and IPv6 on ports 80 and 443
rules_ports_80_443=()
for port in $(echo "${PORTS//,/ }"); do
    rule_ipv4=$(jq -n --arg port "$port" --arg ip "$IPV4" '{"direction": "in", "protocol": "tcp", "port": $port, "source_ips": [$ip], "destination_ips": []}')
    rule_ipv6=$(jq -n --arg port "$port" --arg ip "$IPV6" '{"direction": "in", "protocol": "tcp", "port": $port, "source_ips": [$ip], "destination_ips": []}')
